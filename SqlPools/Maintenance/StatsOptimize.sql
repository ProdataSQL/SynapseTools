...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[admin].[StatsOptimize]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [admin].[StatsOptimize] AS'
END
GO
ALTER PROCEDURE [admin].[IndexOptimize]
